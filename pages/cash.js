...
let _day     = '';   // вибраний день YYYY-MM-DD (історія по днях)
...
.csh-datebar{display:flex;align-items:center;justify-content:center;gap:10px;padding:10px 20px 0}
.csh-nav{width:34px;height:34px;border-radius:10px;background:var(--bg2);border:0.5px solid var(--border);color:var(--text1);font-size:18px;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;font-family:var(--font-h)}
.csh-nav:disabled{opacity:.3}
.csh-date-wrap{position:relative;min-width:150px;text-align:center}
.csh-date-lbl{font-family:var(--font-h);font-size:14px;font-weight:700;color:var(--text0)}
.csh-date-inp{position:absolute;inset:0;width:100%;height:100%;opacity:0;cursor:pointer;border:none}
...
function dateBarHTML() {
  ...
  return `
    <div class="csh-datebar">
      <button class="csh-nav" onclick="window.__cash.dayStep(-1)" aria-label="Попередній день">‹</button>
      <div class="csh-date-wrap">
        <span class="csh-date-lbl">${dayLabel(_day)}</span>
        <input type="date" class="csh-date-inp" value="${_day}" max="${todayIso()}" onchange="window.__cash.pickDay(this.value)">
      </div>
      <button class="csh-nav" onclick="window.__cash.dayStep(1)" ${isToday ? 'disabled' : ''} aria-label="Наступний день">›</button>
    </div>`;
}
...
function bodyHTML() {
  ...
  if (_loading && !_items.length) {
    return dateBarHTML() + `<div class="csh-empty"><div class="csh-empty-txt">Завантаження…</div></div>`;
  }
  const totalLbl = isMgr() ? 'Вилучено з каси' : 'Взято з каси';
  let html = dateBarHTML() + `
    <div class="csh-total">
      <div class="csh-total-val">−${fmtMoney(_total)}</div>
      <div class="csh-total-lbl">${totalLbl} ${isToday ? 'сьогодні' : 'за день'}</div>
    </div>
    ${isToday ? `<button class="csh-add" onclick="window.__cash.openAdd()">Взяв з каси</button>` : ''}`;

  if (!_items.length) {
    html += `<div class="csh-sec">${dayLabel(_day)}</div>
      <div class="csh-empty"><div class="csh-empty-txt">${isToday ? (isMgr() ? 'Сьогодні вилучень з каси не було' : 'Ви ще нічого не брали з каси сьогодні') : 'Цього дня вилучень не було'}</div></div>`;
    return html;
  }

  html += `<div class="csh-sec">${dayLabel(_day)}${isMgr() ? ' · по кому' : ''}</div>`;
...
async function load() {
  ...
  try {
    const res  = await fetch(`${API}/api/cash/withdrawals?venueId=${vId}&day=${_day}`, { headers: { Authorization: `Bearer ${token()}` } });
    const data = await res.json();
...
export default {
  render() {
    _items = []; _total = 0; _loading = true; _addOpen = false; _saving = false; _waiters = []; _editId = null; _confirmId = null; _day = todayIso();
    return `
    ${CSS}
    <div class="csh-wrap">
      <div class="csh-header">
        ...
        <div>
          ...
          <div class="csh-sub">${esc(state.venue || '')}</div>
        </div>
      </div>
  ...
  init() {
    window.__cash = {
      // Навігація по днях (історія)
      dayStep(n) {
        const d = new Date(`${_day}T00:00:00`); d.setDate(d.getDate() + n);
        const iso = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
        if (iso > todayIso()) return;          // не в майбутнє
        _day = iso; load();
      },
      pickDay(val) {
        if (!val) return;
        _day = val > todayIso() ? todayIso() : val;
        load();
      },
