...
// «17 сер 2026» читалось як «середа» — насправді це скорочення МІСЯЦЯ (серпень),
// і воно однакове цілий місяць. Пишемо місяць повністю і додаємо день тижня;
// рік показуємо лише коли він не поточний.
const WDAY_UA  = ['нд','пн','вт','ср','чт','пт','сб'];   // коротко — мітка затиснута між стрілками
...
function fmtDate(d) {
  ...
  return `${day} ${MONTH_UA[m - 1]}${year}, ${WDAY_UA[dt.getDay()]}`;
}
