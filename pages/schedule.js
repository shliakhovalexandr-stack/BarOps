...
// з однаковим імʼям («Шманько Владислав» ≠ «Владислав Шевченко»: спільне «владислав», конфлікт прізвищ).
function nameTokensMatch(a, b) {
  const tok = s => (s || '').toLowerCase().split(/\s+/).map(w => w.replace(/[.,]/g, '')).filter(w => w.length >= 3);
  const eq  = (x, y) => x === y || x.startsWith(y) || y.startsWith(x);   // «влад» ≈ «владислав»
  const ta = tok(a), tb = tok(b);
  if (!ta.length || !tb.length) return false;
  if (!ta.some(x => tb.some(y => eq(x, y)))) return false;               // жодного спільного слова
  const restA = ta.filter(x => !tb.some(y => eq(x, y)));
