...
function prodListHTML() {
  ...
  if (_formMode === 'transfer') {
    // Переміщення ПОБЛАЖЛИВО: склад-джерело + невідома зона (склад не «бар/кухня» — напр. «Загальний»
    // або на La Pasta бар-склад названо інакше → інакше товари типу «Сік в асортименті» ховались).
    // Ховаємо лише ПРОТИЛЕЖНУ зону.
    const src = transferSourceZone();
    list = _prods.filter(p => (!q || p.name.toLowerCase().includes(q)) && (!src || p.zone === src || !p.zone));
  } else {
