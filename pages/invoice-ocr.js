...
let _payWeekday    = null;   // день тижня оплати 1=Пн..7=Нд («оплата по чт» у картці постачальника)
...
function rollToWeekday(iso, wd) {   // вперед до дня тижня (1=Пн..7=Нд); якщо вже цей день — лишаємо
  const d = new Date(iso + 'T00:00:00Z');
  if (isNaN(d) || !wd) return iso;
  const cur = d.getUTCDay() === 0 ? 7 : d.getUTCDay();
  d.setUTCDate(d.getUTCDate() + ((wd - cur + 7) % 7));
  return d.toISOString().slice(0, 10);
}
function termsOf(id) { const m = _catalog.supplierTerms || {}; return (id && m[id]) ? m[id] : null; }   // {days, weekday}
function applyTerms(t) { _payDays = t ? t.days : null; _payWeekday = (t && t.weekday) ? t.weekday : null; }
function recomputeDue() {
  if (_dueTouched) return;
  const dt = ((_invoiceDate || '') + '').trim();
  if ((_payDays == null && !_payWeekday) || !/^\d{4}-\d{2}-\d{2}$/.test(dt)) _dueDate = '';
  else {
    let d = addDays(dt, Math.round(+(_payDays || 0)));
    if (_payWeekday) d = rollToWeekday(d, _payWeekday);
    _dueDate = d;
  }
...
.io-meta select{width:100%;background:none;border:none;color:var(--text0);font-family:var(--font-h);font-size:13px;outline:none;appearance:none;-webkit-appearance:none;padding:0}
...
function reviewView() {
  ...
  return `<div class="io-scroll">
    ...
    ${_isPoster ? '' : `<div class="io-meta-row" style="margin-bottom:${_supplier && _payDays == null && !_payWeekday && !_dueDate ? '4px' : '10px'}">
      <div class="io-meta"><div class="io-meta-l">Відстрочка, дн</div><input id="io-paydays" type="number" min="0" inputmode="numeric" placeholder="—" value="${_payDays != null ? _payDays : ''}" onchange="window.__io.payDays(this.value)"></div>
      <div class="io-meta"><div class="io-meta-l">День оплати</div><select onchange="window.__io.payWeekday(this.value)">${WD_NAMES.map((n, i) => `<option value="${i}"${(_payWeekday || 0) === i ? ' selected' : ''}>${n}</option>`).join('')}</select></div>
...
async function matchAndReview() {
  ...
  // строк оплати: дата з накладної (OCR, якщо надруковано) → дата + умови з памʼяті
...
async function loadCatalog() {
  try {
    ...
    if (res.ok) {
      ...
      if (_payDays == null && !_payWeekday && _supplier) { applyTerms(termsOf(_supplier.id)); recomputeDue(); }
...
async function termsImport() {
  ...
  try {
    ...
    const fmt = (t) => t.weekday ? `${WD_NAMES[t.weekday]}${t.days ? ' +' + t.days + ' дн' : ''}` : `${t.days} дн`;
    const lines = list.slice(0, 20).map(f => `• ${f.name}: ${fmt(f)} (${f.samples} док)${f.existing != null ? ` — вже задано ${fmt(f.existing)}, лишаю` : ''}`).join('\n');
    ...
    if (_catalog.supplierTerms) for (const f of fresh) _catalog.supplierTerms[f.supplierId] = { days: f.days, weekday: f.weekday || null };
